...
public class Mongo {
    ...
    public static boolean login(String usuario, String senha){
        ...
        query.put("_id", usuario);
        query.put("senha", senha);
        FindIterable<Document> iterable = MY_DB.getCollection("login").find(query);
        for(Document document : iterable){
            if(document.getString("_id").equals(usuario) && document.getString("senha").equals(senha))
                return true;
    ...
    public static String construaLinhaDoTempoDoDocente(String id){   
        ...
        */
        ...
        linhaDoTempoDoDocente.append("<table id=\"timeline\" border=\"0\" bordercolor=\"#F0F8FF	 \" cellspacing=\"1\" cellpadding=\"0\" style=\"margin: 0 auto;\">\n");
        ...
        for(int ano : listaAnos){
            linhaDoTempoDoDocente.append("  <td align=middle>&nbsp</td>\n");
            linhaDoTempoDoDocente.append("  <td align=middle>&nbsp</td>\n");
            linhaDoTempoDoDocente.append("  <td align=middle>&nbsp</td>\n");
            if(ano < maiorAno)
                linhaDoTempoDoDocente.append("  <td></td>\n");
        ...
        for(String chave : chaves){
            ...
            for(int ano : listaAnos){
                if(Integer.parseInt(discente.get("anoAdmissao")) <= ano && Integer.parseInt(discente.get("anoConclusao")) >= ano){
                    ...
                    linhaDoTempoDoDocente.append("  <td width=20 ></td>\n");
                    linhaDoTempoDoDocente.append("  <td width=20 ></td>\n");
                    linhaDoTempoDoDocente.append("  <td width=20 ></td>\n");
                    if(ano < maiorAno)linhaDoTempoDoDocente.append("  <td width=20></td>\n");
                }    
        ...
        linhaDoTempoDoDocente.append("<tr><td>&nbsp</td></tr><tr>\n");
        ...
        for(int ano : listaAnos){
            ...
            for(String chave : chaves){
                TreeMap<String, String> discente = discentes.get(chave);
                if(ano >= Integer.parseInt(discente.get("anoAdmissao")) && ano <= Integer.parseInt(discente.get("anoConclusao"))){
                    contador++;
                }
            }
            linhaDoTempoDoDocente.append("  <td colspan=\"3\" align=middle><span style=\"color: red\">").append(contador).append("</span></td>\n");
            if(ano < maiorAno)
                linhaDoTempoDoDocente.append("  <td></td>\n");
        }
